...
#define BUTTON0	(PORTD, 7)
#define BUTTON1	(PORTD, 6)
#define BUTTON2	(PORTB, 0)
...
uint8_t check_button(void)
{
	...
	if( BLUE_BUTTON == 0) button_pressed |= (1<<0); //Button 0
	if( YELLOW_BUTTON == 0) button_pressed |= (1<<1); //Button 1; Yellow
	if( (PINB & (1<<0)) == 0) button_pressed |= (1<<2); //Button 2
	if( (PINB & (1<<1)) == 0) button_pressed |= (1<<3); //Button 3
